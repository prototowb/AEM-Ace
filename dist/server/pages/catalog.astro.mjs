...
const _sfc_main = /* @__PURE__ */ vueExports.defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    __expose();
    const props = __props;
    const selectedCategory = vueExports.ref(null);
    const searchQuery = vueExports.ref("");
    const questions = vueExports.ref([]);
    const offset = vueExports.ref(0);
    const hasMore = vueExports.ref(true);
    const isLoading = vueExports.ref(false);
    const userVotes = vueExports.ref({});
    const sessionId = vueExports.ref("");
    ...
    const filteredQuestions = vueExports.computed(() => {
    ...
    vueExports.watch([selectedCategory, searchQuery], () => {
      offset.value = 0;
      hasMore.value = true;
      fetchQuestions(true);
    });
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${serverRenderer_cjs_prodExports.ssrRenderAttrs(_attrs)}><div class="mb-8"><div class="flex flex-wrap gap-4 justify-center"><!--[-->`);
  serverRenderer_cjs_prodExports.ssrRenderList($setup.props.categories, (category) => {
    ...
    ])}">${serverRenderer_cjs_prodExports.ssrInterpolate(category.name)} <span class="ml-2 text-sm opacity-75">(${serverRenderer_cjs_prodExports.ssrInterpolate(category.questionCount)})</span></button>`);
  });
  _push(`<!--]--><button class="${serverRenderer_cjs_prodExports.ssrRenderClass([
    "px-4 py-2 rounded-lg font-medium transition-colors duration-200",
    $setup.selectedCategory === null ? "bg-primary text-white" : "bg-white text-gray-700 hover:bg-gray-50 border border-gray-200"
  ])}"> All Questions </button></div></div><div class="mb-8"><div class="max-w-md mx-auto"><div class="relative"><div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><svg class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></div><input${serverRenderer_cjs_prodExports.ssrRenderAttr("value", $setup.searchQuery)} type="text" placeholder="Search questions..." class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary focus:border-primary"></div></div></div><div class="grid gap-6"><!--[-->`);
  serverRenderer_cjs_prodExports.ssrRenderList($setup.filteredQuestions, (question) => {
    _push(`<div class="card"><div class="flex justify-between items-start mb-4"><div><h3 class="text-lg font-heading font-semibold text-gray-900 mb-2">${serverRenderer_cjs_prodExports.ssrInterpolate(question.title)} `);
    ...
    if (question.category) {
      _push(`<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style="${serverRenderer_cjs_prodExports.ssrRenderStyle({ backgroundColor: question.category.color + "20", color: question.category.color })}">${serverRenderer_cjs_prodExports.ssrInterpolate(question.category.name)}</span>`);
    } else {
      _push(`<!---->`);
    }
    if (question.isMultipleChoice) {
      _push(`<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">${serverRenderer_cjs_prodExports.ssrInterpolate(question.correctAnswers.length)} correct answers </span>`);
    } else {
      _push(`<!---->`);
    }
    _push(`</div></div><span class="${serverRenderer_cjs_prodExports.ssrRenderClass([
    ...
    ])}">${serverRenderer_cjs_prodExports.ssrInterpolate(question.difficulty)}</span></div><div class="mb-4 flex items-center gap-3"><button class="${serverRenderer_cjs_prodExports.ssrRenderClass([$setup.userVotes[question._id] === 1 ? "bg-green-600 text-white border-green-600" : "bg-white text-gray-700 border-gray-200 hover:bg-gray-50", "inline-flex items-center justify-center rounded-md border px-2 py-1 text-sm transition-colors"])}" aria-label="Upvote"> \u25B2 </button><span class="${serverRenderer_cjs_prodExports.ssrRenderClass([(question.voteScore || 0) >= 0 ? "text-gray-900" : "text-gray-700", "min-w-[3rem] text-center font-medium"])}">${serverRenderer_cjs_prodExports.ssrInterpolate(question.voteScore ?? 0)}</span><button class="${serverRenderer_cjs_prodExports.ssrRenderClass([$setup.userVotes[question._id] === -1 ? "bg-red-600 text-white border-red-600" : "bg-white text-gray-700 border-gray-200 hover:bg-gray-50", "inline-flex items-center justify-center rounded-md border px-2 py-1 text-sm transition-colors"])}" aria-label="Downvote"> \u25BC </button><span class="ml-2 text-xs text-gray-500">${serverRenderer_cjs_prodExports.ssrInterpolate(question.upvotes ?? 0)} up \u2022 ${serverRenderer_cjs_prodExports.ssrInterpolate(question.downvotes ?? 0)} down </span></div><div class="mb-4"><p class="text-gray-700 mb-4">${serverRenderer_cjs_prodExports.ssrInterpolate(question.question)}</p><div class="space-y-2"><!--[-->`);
    serverRenderer_cjs_prodExports.ssrRenderList(question.options, (option, index) => {
      ...
      ])}"><span class="font-medium">${serverRenderer_cjs_prodExports.ssrInterpolate(String.fromCharCode(65 + index))}.</span> ${serverRenderer_cjs_prodExports.ssrInterpolate(option)} `);
    ...
    if (question.explanation) {
      _push(`<div class="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200"><h4 class="font-medium text-blue-900 mb-2">Explanation:</h4><p class="text-blue-800">${serverRenderer_cjs_prodExports.ssrInterpolate(question.explanation)}</p></div>`);
    } else {
      _push(`<!---->`);
    }
    if (question.tags && question.tags.length > 0) {
      _push(`<div class="mt-4 flex flex-wrap gap-2"><!--[-->`);
      serverRenderer_cjs_prodExports.ssrRenderList(question.tags, (tag) => {
        _push(`<span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-800">${serverRenderer_cjs_prodExports.ssrInterpolate(tag)}</span>`);
      });
      _push(`<!--]--></div>`);
    } else {
      _push(`<!---->`);
