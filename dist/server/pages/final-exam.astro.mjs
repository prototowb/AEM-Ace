...
const _sfc_main = /* @__PURE__ */ vueExports.defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    __expose();
    const isStarted = vueExports.ref(false);
    const isComplete = vueExports.ref(false);
    const loading = vueExports.ref(false);
    const size = vueExports.ref(20);
    const questions = vueExports.ref([]);
    const index = vueExports.ref(0);
    const selected = vueExports.ref([]);
    const answered = vueExports.ref(false);
    const userAnswers = vueExports.ref([]);
    const remainingSeconds = vueExports.ref(0);
    let countdownTimer;
    const current = vueExports.computed(() => questions.value[index.value]);
    const correct = vueExports.computed(() => userAnswers.value.filter((ans, i) => {
    ...
    const formattedTimeLeft = vueExports.computed(() => {
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  if (!$setup.isStarted) {
    _push(`<div class="max-w-2xl mx-auto"><div class="card text-center"><div class="mb-6"><label class="block text-sm font-medium text-gray-700 mb-3">Number of Questions</label><select class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary"><option value="10"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.size) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.size, "10") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.size, "10")) ? " selected" : ""}>10 Questions</option><option value="20"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.size) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.size, "20") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.size, "20")) ? " selected" : ""}>20 Questions</option><option value="30"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.size) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.size, "30") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.size, "30")) ? " selected" : ""}>30 Questions</option><option value="50"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.size) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.size, "50") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.size, "50")) ? " selected" : ""}>50 Questions</option></select></div><button${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr($setup.loading) ? " disabled" : ""} class="btn-primary disabled:opacity-50 disabled:cursor-not-allowed">`);
    if ($setup.loading) {
    ...
    _push(`<div class="max-w-3xl mx-auto"><div class="mb-8"><div class="flex justify-between text-sm text-gray-600 mb-2"><span>Question ${serverRenderer_cjs_prodExports.ssrInterpolate($setup.index + 1)} of ${serverRenderer_cjs_prodExports.ssrInterpolate($setup.questions.length)}</span><div class="flex items-center gap-4"><span>${serverRenderer_cjs_prodExports.ssrInterpolate(Math.round(($setup.index + 1) / $setup.questions.length * 100))}% Complete</span><span class="${serverRenderer_cjs_prodExports.ssrRenderClass($setup.remainingSeconds <= 300 ? "text-red-600 font-semibold" : "text-gray-800")}">\u23F1\uFE0F ${serverRenderer_cjs_prodExports.ssrInterpolate($setup.formattedTimeLeft)}</span></div></div><div class="w-full bg-gray-200 rounded-full h-2"><div class="bg-primary h-2 rounded-full transition-all duration-300" style="${serverRenderer_cjs_prodExports.ssrRenderStyle({ width: ($setup.index + 1) / $setup.questions.length * 100 + "%" })}"></div></div></div><div class="card"><div class="mb-6"><div class="flex justify-between items-start mb-4"><h2 class="text-xl font-heading font-semibold text-gray-900">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.current.title)} `);
    if ($setup.current.isMultipleChoice) {
      _push(`<span class="ml-2 text-sm text-blue-600 font-normal">(Select Multiple)</span>`);
    } else {
      _push(`<!---->`);
    }
    _push(`</h2><span class="${serverRenderer_cjs_prodExports.ssrRenderClass(["inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium", $setup.current.difficulty === "beginner" ? "bg-green-100 text-green-800" : $setup.current.difficulty === "intermediate" ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"])}">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.current.difficulty)}</span></div><p class="text-gray-700 text-lg">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.current.question)}</p></div><div class="space-y-3 mb-6"><!--[-->`);
    serverRenderer_cjs_prodExports.ssrRenderList($setup.current.options, (option, i) => {
      _push(`<button${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr($setup.answered) ? " disabled" : ""} class="${serverRenderer_cjs_prodExports.ssrRenderClass(["quiz-option w-full text-left", $setup.selected.includes(i) ? "selected" : "", $setup.answered && $setup.current.correctAnswers.includes(i) ? "correct" : "", $setup.answered && $setup.selected.includes(i) && !$setup.current.correctAnswers.includes(i) ? "incorrect" : ""])}"><div class="flex items-center"><div class="mr-3 flex-shrink-0">`);
      if ($setup.current.isMultipleChoice) {
        _push(`<div class="${serverRenderer_cjs_prodExports.ssrRenderClass(["w-4 h-4 border-2 rounded flex items-center justify-center", $setup.selected.includes(i) ? "bg-blue-500 border-blue-500" : "border-gray-300"])}">`);
        ...
        _push(`<div class="${serverRenderer_cjs_prodExports.ssrRenderClass(["w-4 h-4 border-2 rounded-full flex items-center justify-center", $setup.selected.includes(i) ? "bg-blue-500 border-blue-500" : "border-gray-300"])}">`);
        if ($setup.selected.includes(i)) {
      ...
      _push(`</div><div class="flex-1"><span class="font-medium">${serverRenderer_cjs_prodExports.ssrInterpolate(String.fromCharCode(65 + i))}.</span> ${serverRenderer_cjs_prodExports.ssrInterpolate(option)}</div>`);
      if ($setup.answered) {
