...
const _sfc_main = /* @__PURE__ */ vueExports.defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const submitError = vueExports.ref("");
    const tagsInput = vueExports.ref("");
    vueExports.watch(tagsInput, (val) => {
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${serverRenderer_cjs_prodExports.ssrRenderAttrs(vueExports.mergeProps({ class: "max-w-2xl mx-auto" }, _attrs))}><div class="card"><h2 class="text-2xl font-heading font-bold text-gray-900 mb-6">Submit a Question</h2><p class="text-gray-600 mb-6"> Submit your own quiz question. Submissions are reviewed before appearing in the catalog. </p><form novalidate><input${serverRenderer_cjs_prodExports.ssrRenderAttr("value", $setup.form.website)} type="text" class="hidden" autocomplete="off" tabindex="-1" aria-hidden="true"><div class="grid gap-6"><div><label class="block text-sm font-medium text-gray-700 mb-2">Title</label><input${serverRenderer_cjs_prodExports.ssrRenderAttr("value", $setup.form.title)} type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary" required>`);
  if ($setup.errors.title) {
    _push(`<p class="mt-1 text-sm text-red-600">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.errors.title)}</p>`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div><div><label class="block text-sm font-medium text-gray-700 mb-2">Question</label><textarea rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary" required>${serverRenderer_cjs_prodExports.ssrInterpolate($setup.form.question)}</textarea>`);
  if ($setup.errors.question) {
    _push(`<p class="mt-1 text-sm text-red-600">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.errors.question)}</p>`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div><div><div class="flex items-center justify-between mb-2"><label class="block text-sm font-medium text-gray-700">Answer Options</label><div class="flex gap-2"><button type="button" class="text-sm text-primary hover:underline"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr($setup.form.options.length >= 6) ? " disabled" : ""}>Add option</button><span class="text-xs text-gray-500">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.form.options.length)}/6</span></div></div><div class="space-y-3"><!--[-->`);
  serverRenderer_cjs_prodExports.ssrRenderList($setup.form.options, (opt, idx) => {
    _push(`<div class="flex items-start gap-3"><input type="checkbox"${serverRenderer_cjs_prodExports.ssrRenderAttr("id", "correct-" + idx)} class="mt-2"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr($setup.form.correctAnswers.includes(idx)) ? " checked" : ""}><div class="flex-1"><div class="flex gap-2"><input${serverRenderer_cjs_prodExports.ssrRenderAttr("value", $setup.form.options[idx])}${serverRenderer_cjs_prodExports.ssrRenderAttr("placeholder", `Option ${String.fromCharCode(65 + idx)}`)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary"><button type="button" class="px-2 py-2 text-sm text-gray-500 hover:text-red-600"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr($setup.form.options.length <= 2) ? " disabled" : ""}>Remove</button></div><label${serverRenderer_cjs_prodExports.ssrRenderAttr("for", "correct-" + idx)} class="text-xs text-gray-600">Check if this option is correct</label></div></div>`);
  });
  _push(`<!--]--></div>`);
  if ($setup.errors.options) {
    _push(`<p class="mt-1 text-sm text-red-600">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.errors.options)}</p>`);
  } else {
    _push(`<!---->`);
  }
  if ($setup.errors.correctAnswers) {
    _push(`<p class="mt-1 text-sm text-red-600">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.errors.correctAnswers)}</p>`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div><div><label class="block text-sm font-medium text-gray-700 mb-2">Explanation (optional)</label><textarea rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.form.explanation)}</textarea></div><div><label class="block text-sm font-medium text-gray-700 mb-2">Category</label><select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary" required><option value=""${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.form.categoryId) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.form.categoryId, "") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.form.categoryId, "")) ? " selected" : ""}>Select a category</option><!--[-->`);
  serverRenderer_cjs_prodExports.ssrRenderList($setup.props.categories, (cat) => {
    _push(`<option${serverRenderer_cjs_prodExports.ssrRenderAttr("value", cat._id)}${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.form.categoryId) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.form.categoryId, cat._id) : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.form.categoryId, cat._id)) ? " selected" : ""}>${serverRenderer_cjs_prodExports.ssrInterpolate(cat.name)}</option>`);
  });
  _push(`<!--]--></select>`);
  if ($setup.errors.categoryId) {
    _push(`<p class="mt-1 text-sm text-red-600">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.errors.categoryId)}</p>`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div><div><label class="block text-sm font-medium text-gray-700 mb-2">Difficulty</label><select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary" required><option value=""${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.form.difficulty) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.form.difficulty, "") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.form.difficulty, "")) ? " selected" : ""}>Select difficulty</option><option value="beginner"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.form.difficulty) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.form.difficulty, "beginner") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.form.difficulty, "beginner")) ? " selected" : ""}>Beginner</option><option value="intermediate"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.form.difficulty) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.form.difficulty, "intermediate") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.form.difficulty, "intermediate")) ? " selected" : ""}>Intermediate</option><option value="advanced"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(Array.isArray($setup.form.difficulty) ? serverRenderer_cjs_prodExports.ssrLooseContain($setup.form.difficulty, "advanced") : serverRenderer_cjs_prodExports.ssrLooseEqual($setup.form.difficulty, "advanced")) ? " selected" : ""}>Advanced</option></select>`);
  if ($setup.errors.difficulty) {
    _push(`<p class="mt-1 text-sm text-red-600">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.errors.difficulty)}</p>`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div><div><label class="block text-sm font-medium text-gray-700 mb-2">Tags (comma-separated, optional)</label><input${serverRenderer_cjs_prodExports.ssrRenderAttr("value", $setup.tagsInput)} type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary" placeholder="e.g. components, sling, dispatcher"></div><div class="grid md:grid-cols-2 gap-4"><div><label class="block text-sm font-medium text-gray-700 mb-2">Your Name (optional)</label><input${serverRenderer_cjs_prodExports.ssrRenderAttr("value", $setup.form.submitterName)} type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary"></div><div><label class="block text-sm font-medium text-gray-700 mb-2">Your Email (optional)</label><input${serverRenderer_cjs_prodExports.ssrRenderAttr("value", $setup.form.submitterEmail)} type="email" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary"></div></div><div class="flex items-center gap-3"><button type="submit" class="btn-primary disabled:opacity-50"${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr($setup.submitting) ? " disabled" : ""}>`);
  if ($setup.submitting) {
  ...
  if ($setup.successMessage) {
    _push(`<span class="text-green-600">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.successMessage)}</span>`);
  } else {
    _push(`<!---->`);
  }
  if ($setup.submitError) {
    _push(`<span class="text-red-600">${serverRenderer_cjs_prodExports.ssrInterpolate($setup.submitError)}</span>`);
  } else {
    _push(`<!---->`);
  }
