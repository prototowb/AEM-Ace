...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const title = typeof body?.title === 'string' && body.title.trim() ? String(body.title) : undefined;
    ...
    // Deduplicate by id and trim to desired size
    // Shuffle in JS for randomness
    ...
    const doc = {
      ...
      title: title || `Final Exam ${new Date().toISOString()}`,
...
// Read latest or specific paper
export const GET: APIRoute = async ({ url }) => {
  try {
    const id = url.searchParams.get('id') || '';
    const query = id
      ? `*[_type=='finalExamPaper' && _id==$id][0]{ _id, title, generatedAt, criteria, questions[]-> { _id, title, question, options, correctAnswers, isMultipleChoice, difficulty, category->{_id,name,color} } }`
      : `*[_type=='finalExamPaper'] | order(generatedAt desc) [0]{ _id, title, generatedAt, criteria, questions[]-> { _id, title, question, options, correctAnswers, isMultipleChoice, difficulty, category->{_id,name,color} } }`;
